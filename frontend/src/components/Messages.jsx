...
const useSendMessage = () => {
    ...
    const sendMessage = async (message) => {
        setLoading(true);
        try {
            const res = await axios.post(`https://estudiantes12.backendprueba.xyz/mensajes/enviar/${selectedConversation._id}`, { message, senderId: userId });
            const data = res.data;
...
const useGetMessages = () => {
    ...
    useEffect(() => {
        const getMessages = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`https://estudiantes12.backendprueba.xyz/mensajes/${selectedConversation._id}/${userId}`);
                const data = res.data;
                if (data.error) throw new Error(data.error);
                setMessages(data);
            } catch (error) {
