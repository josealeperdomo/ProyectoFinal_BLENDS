...
export function DetailsUser() {
  ...
  useEffect(() => {
    const obtenerUsuario = async () => {
      try {
        const response = await axios.get(`https://estudiantes12.backendprueba.xyz/users/${id}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`https://estudiantes12.backendprueba.xyz/users/${id}`, user);
      Swal.fire({
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`https://estudiantes12.backendprueba.xyz/users/${id}`);
      Swal.fire({
