...
export function PerfilUser() {
  ...
  useEffect(() => {
    const obtenerPerfilUsuario = async () => {
      try {
        const response = await axios.get(`https://estudiantes12.backendprueba.xyz/users/user/${user}`);
        setPerfilUsuario(response.data);

        const publicacionesResponse = await axios.get(`https://estudiantes12.backendprueba.xyz/publicaciones/user/${response.data._id}`);
  ...
  useEffect(() => {
    const fetchPublicaciones = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`https://estudiantes12.backendprueba.xyz/publicaciones/user/${perfilUsuario._id}`);
  ...
  const handleLike = async (publicacionId) => {
    try {
        const response = await axios.post(`https://estudiantes12.backendprueba.xyz/likes/publicaciones/${publicacionId}/like`, {
            id_usuario: userId
        });
        setLikes(prevLikes => ({
            ...prevLikes,
  ...
  const handleUnlike = async (publicacionId) => {
    try {
        const response = await axios.delete(`https://estudiantes12.backendprueba.xyz/likes/publicaciones/${publicacionId}/unlike/${userId}`);
        setLikes(prevLikes => ({
            ...prevLikes,
            [publicacionId]: response.data.cantidad_likes
        }));
